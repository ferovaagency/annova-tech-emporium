...
export default function AIChatWidget() {
  ...
  useEffect(() => { if (!loading && open) setTimeout(() => inputRef.current?.focus(), 100); }, [loading, open, messages]);
  ...
  const parseMessage = (content: string) => {
    ...
    if (prodMatch) {
      ...
      if (matchedProducts.length > 0) parts.push(<div key="prods" className="space-y-2 my-2">{matchedProducts.map((p) => <div key={p.id} className="bg-background border rounded-lg p-2 flex items-center gap-2">{p.images?.[0] && <img src={p.images[0]} alt={p.name} className="w-12 h-12 rounded object-cover flex-shrink-0" />}<div className="flex-1 min-w-0"><p className="text-xs font-medium line-clamp-1">{p.name}</p><p className="text-xs font-bold text-primary">{formatPrice(p.sale_price || p.price)}</p></div><div className="flex gap-1 flex-shrink-0"><button onClick={() => addToCart({ id: p.id, name: p.name, slug: p.slug, price: p.sale_price || p.price, oldPrice: p.sale_price ? p.price : undefined, image: p.images?.[0] || '', images: p.images || [], category: p.category || '', categorySlug: '', brand: '', condition: 'Nuevo' as any, shortDescription: '', description: '', specs: {}, rating: 5, reviews: 0 })} className="p-1.5 bg-primary text-primary-foreground rounded text-xs" title="Agregar al carrito"><ShoppingCart className="w-3 h-3" /></button><Link to={`/producto/${p.slug}`} className="p-1.5 bg-muted rounded text-xs" title="Ver producto"><ExternalLink className="w-3 h-3" /></Link></div></div>)}</div>);
    }
