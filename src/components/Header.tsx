      ...
      <div className="bg-background border-b shadow-sm">
        <div className="container mx-auto flex items-center gap-4 px-[20px] py-[30px]">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <Link to="/mi-cuenta" className="p-2 hover:bg-muted rounded-lg transition-colors"><User className="w-5 h-5" /></Link>
      ...
      <nav className="bg-accent text-accent-foreground hidden lg:block">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-1">
            ...
            {categories.map((cat) => (
              <Link key={cat.slug} to={`/tienda?categoria=${cat.slug}`} className="py-2.5 px-4 text-sm font-medium hover:bg-primary hover:text-primary-foreground transition-colors whitespace-nowrap rounded-sm">
                {cat.name}
              </Link>
            ))}
        ...
        >
          <div className="container mx-auto px-4 py-6 grid grid-cols-3 gap-6">
            {categories.length > 0 ? categories.map((cat) => (
              <Link key={cat.slug} to={`/tienda?categoria=${cat.slug}`} className="flex items-center gap-4 p-4 rounded-lg hover:bg-muted transition-colors group">
            ...
            )) : <p className="text-sm text-muted-foreground px-4">No hay categorías disponibles.</p>}
          </div>
        </div>
      )}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-background border-b shadow-lg">
          <div className="p-4">
            ...
            {categories.length > 0 ? categories.map((cat) => (
              <Link key={cat.slug} to={`/tienda?categoria=${cat.slug}`} className="flex items-center gap-3 py-3 px-2 border-b border-border hover:bg-muted transition-colors" onClick={() => setMobileMenuOpen(false)}>
                <span className="text-xl">{cat.icon}</span>
                <span className="font-medium text-sm">{cat.name}</span>
              </Link>
            )) : <p className="py-3 px-2 text-sm text-muted-foreground border-b border-border">No hay categorías disponibles.</p>}
