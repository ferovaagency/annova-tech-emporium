...
export default function Index() {
  ...
  return (
    <main>
          ...
          <div className="absolute inset-0">
            <div className="container mx-auto flex h-full items-center px-4">
              <div className="max-w-2xl text-foreground">
                <h1 className="mb-4 text-4xl leading-none md:text-6xl lg:text-7xl font-bebas">Tecnología<br /><span className="text-primary">Empresarial</span><br />al Mejor Precio</h1>
                ...
                <div className="flex flex-wrap gap-4">
                  <Link to="/tienda" className="inline-flex items-center gap-2 rounded-lg bg-primary px-8 py-3 text-lg font-semibold text-primary-foreground hover:opacity-90 transition-opacity">Comprar Ahora <ArrowRight className="h-5 w-5" /></Link>
                  <a href={getWhatsAppUrl('Necesito cotización empresarial de AnnovaSoft')} target="_blank" rel="noopener noreferrer" className="rounded-lg border-2 border-foreground/20 px-8 py-3 text-lg font-semibold text-foreground hover:border-foreground/40 transition-colors">Cotizar</a>
                </div>
              </div>
            </div>
          </div>
          ...
          {heroBanners.length > 1 && (
            <>
              ...
              <div className="absolute bottom-6 left-1/2 flex -translate-x-1/2 gap-2">
                {heroBanners.map((banner, index) => (
                  <button key={banner.src} onClick={() => goToBanner(index)} className={`h-2.5 w-2.5 rounded-full transition-all ${index === heroIndex ? 'bg-primary scale-110' : 'bg-background/70'}`} aria-label={`Ir al banner ${index + 1}`} />
                ))}
              </div>
            </>
          )}
      ...
      <section className="border-t bg-muted py-16"><div className="container mx-auto px-4 text-center"><h2 className="mb-4 text-3xl md:text-5xl font-bebas">¿Necesitas Equipar Tu Empresa?</h2><p className="mx-auto mb-8 max-w-2xl text-lg text-muted-foreground font-montserrat">Solicita una cotización personalizada y recibe asesoría de nuestros expertos en tecnología empresarial.</p><a href={getWhatsAppUrl('Hola, necesito una cotización empresarial de AnnovaSoft')} target="_blank" rel="noopener noreferrer" className="inline-block rounded-lg bg-primary px-10 py-4 text-lg font-bold text-primary-foreground hover:opacity-90 transition-opacity">Solicitar Cotización</a></div></section>
    </main>
  );
}
