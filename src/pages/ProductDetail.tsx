...
export default function ProductDetail() {
  ...
  const images = product.images && product.images.length > 0 ? product.images : ['https://placehold.co/400x400?text=Sin+imagen'];
  ...
  const cartProduct = {
    ...
    categorySlug: '',
    brand: product.brand || '',
    condition: (product.condition || 'Nuevo') as any,
    ...
    <main className="py-8">
      <div className="container mx-auto px-4">
        ...
        <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          ...
          {product.category && (
            <>
              <Link to={`/tienda?categoria=${product.category}`} className="hover:text-foreground">{product.category}</Link>
              <ChevronRight className="w-3 h-3" />
            </>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="secondary" className="text-xs">{product.condition || 'Nuevo'}</Badge>
              {product.warranty && <Badge className="bg-primary text-primary-foreground text-xs">{product.warranty}</Badge>}
            </div>
            ...
            {reviews.length > 0 && (
              <div className="flex items-center gap-2 mb-4">
                {[...Array(5)].map((_, i) => {
                  ...
                  return <Star key={i} className={`w-4 h-4 ${i < Math.floor(avg) ? 'fill-yellow-400 text-yellow-400' : 'text-muted'}`} />;
                })}
            ...
            <div className="flex items-baseline gap-3 mb-4">
              <span className="text-3xl font-bold text-primary">{formatPrice(product.sale_price || product.price)}</span>
              {product.sale_price && <span className="text-lg text-muted-foreground line-through">{formatPrice(product.price)}</span>}
            </div>
            ...
            {/* Availability notice */}
            <div className="bg-primary/10 border border-primary/30 rounded-lg p-3 mb-4 mt-4 flex items-start gap-2">
            ...
            <div className="space-y-3 mb-8">
              <button onClick={() => addToCart(cartProduct)} className="w-full bg-primary text-primary-foreground font-bold py-3 rounded-lg text-lg flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
        ...
        <Tabs defaultValue="descripcion" className="mb-12">
          <TabsList className="w-full justify-start">
            ...
            <TabsTrigger value="resenas">Reseñas ({reviews.length})</TabsTrigger>
          </TabsList>
        ...
        {related.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {related.map(p => {
                const relatedCartProduct = {
                  id: p.id, name: p.name, slug: p.slug, price: p.sale_price || p.price,
                  oldPrice: p.sale_price ? p.price : undefined,
                  image: (p.images && p.images[0]) || '', images: p.images || [],
                  category: p.category || '', categorySlug: '', brand: p.brand || '',
                  condition: (p.condition || 'Nuevo') as any,
                  shortDescription: p.short_description || '', description: '', specs: {},
                  rating: 5, reviews: 0,
                };
                return <ProductCard key={p.id} product={relatedCartProduct} />;
              })}
            </div>
          </section>
        )}
